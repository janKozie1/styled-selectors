...
type Select<T, Props> = {
  ...
  <U>(cb: (curent:T, componentProps: Props) => U): (componentProps: Props) => U
}
...
const wrapper = <T, ComponentProps>(keys: Key[]) => {
    ...
    function select<U>(next: (current: T, componentProps: ComponentProps) => U): (componentProps: ComponentProps) => U
    function select<U extends ComponentProps | keyof T>(next: U) {
        if (isCallback<T, ComponentProps>(next)){
            return (componentProps: ComponentProps) => next(access<T>(keys, componentProps), componentProps)
        }
